...
void queue_next_cmd_req(storj_api_t *storj_api)
{
    ...
    {
        printf("expt resp = %s; rcvd resp = %s \n",storj_api->excp_cmd_resp, storj_api->rcvd_cmd_resp );
        printf("last cmd = %s; cur cmd = %s; next cmd = %s\n",
               storj_api->last_cmd_req, storj_api->curr_cmd_req, storj_api->next_cmd_req);
        if ((storj_api->next_cmd_req != NULL) && (strcmp(storj_api->next_cmd_req, "list-files-req") == 0x00))
        {
            storj_api->last_cmd_req  = storj_api->curr_cmd_req;
