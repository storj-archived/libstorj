...
static int serialize_download_state( storj_download_state_t *state)
{
    ...
    if (fcntl(fileno(state->destination), F_GETPATH, filePath) != -1) {
        ...
        printf("[%s][%d] Unable to create downlaod json file \n",
    ...
    for (int i = 0x00; i < state->total_pointers; i++) {
        ...
        if (state->pointers->shard_hash) {
            json_object_object_add(jptr, "shard_hash", 
        ...
        json_object_object_add(jptr, "downlaoded_size", 
                           json_object_new_int64(state->pointers->downloaded_size));
        if (state->pointers->farmer_id) {
            json_object_object_add(jptr, "farmer_id", 
